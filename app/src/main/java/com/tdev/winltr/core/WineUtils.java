    ...
    public static void overrideWinComponentDlls(Context context, Container container, String wincomponents) {
        ...
        try (WineRegistryEditor registryEditor = new WineRegistryEditor(userRegFile)) {
            ...
            for (String[] wincomponent : new KeyValueSet(wincomponents)) {
                // Guard against mismatched list sizes to avoid NoSuchElementException
                if (oldWinComponentsIter.hasNext() && wincomponent[1].equals(oldWinComponentsIter.next()[1])) continue;
